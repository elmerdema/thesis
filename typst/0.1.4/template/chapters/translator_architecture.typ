...
#let translator_architecture() = [
  ...
  The Translator operates primarily on Data-Plane Telemetry Architecture (#abbr("dta")) packets. Upon ingress, the custom `SwitchIngressParser` inspects the packet headers. If it detects a #abbr("udp") packet destined for port `40040` with the Marina opcode (`0x05`), it transitions into specialized parsing states to extract the `dta_keyval_static_h` and `keywrite_data_h` headers.

  Once parsed, the `FeatureExtraction` control block maps the payload into the `local_metadata_t` structure. While features like `packet_count`, `ps_sum`, and `iat_sum` are extracted directly from the payload, the Translator dynamically calculates `jitter` on the fly. It does this by subtracting the flow's `last_packet_timestamp` (carried in the #abbr("dta") header) from the switch's local hardware ingress timestamp (`ig_intr_md.ingress_mac_tstamp`).
