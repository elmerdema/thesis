#let abstract() = [
  ...
  While papers, such as ViCrypt , have demonstrated that network statistics can predict user QoE, its reliance on end-host GPUs for processing features makes it impossible for direct implementation in the network data plane @wassermann_vicrypt_video_qoe. To bridge this critical gap, this thesis addresses the question whether it is possible to implement Machine Learning models for QoE prediction directly within the network data plane to provide real-time monitoring of encrypted video traffic.
