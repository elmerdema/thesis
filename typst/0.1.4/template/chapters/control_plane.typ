#let control_plane_configuration() = [
  ...
  #figure(
    ...
    caption: [BFRT gRPC entry insertion flow: a match key (destination IP prefix) and action data (egress port and MAC addresses) are constructed separately and passed to \texttt{entry\_add()}, which writes the forwarding rule into the Tofino match-action table.],
  )
  ...
  #block(fill: luma(240), inset: 12pt, radius: 6pt, width: 100%, breakable: true)[
    ...
    into `SwitchIngress.tbl_forward`. The entry matches all traffic destined for
    the configured target IP (prefix length 32) and rewrites both Ethernet
    ...
    #table(
      ...
      [Destination IP], [`192.168.100.1/32`], [Match key in the forward table],
      [Egress port], [`16` (physical 24)], [Tofino port toward TRex Rx],
      [Source MAC], [`D0:77:CE:2B:20:54`], [Tofino port 24 MAC (rewritten)],
      [Destination MAC], [`10:70:fd:30:80:d1`], [TRex port 1 MAC (rewritten)],
      [gRPC endpoint], [`localhost:50052`], [BF Runtime control-plane server],
      [Pipeline], [`p4_marina_reporter`], [P4 program bound at runtime],
    )
  ]
  #figure(
