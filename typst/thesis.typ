...
While papers, such as ViCrypt , have demonstrated that network statistics can predict user QoE, its reliance on end-host GPUs for processing features makes it impossible for direct implementation in the network data plane[2]. The computational and memory constraints of programmable switches prohibit such resource-intensive tasks. To bridge this critical gap, this thesis introduces a split-inference architecture for real-time QoE monitoring. This design strategically divides the predictive model into two components: a module deployed on a P4/Tofino switch performs initial video flow classification and extracts primitive features at line rate, while the control plane calculates the  to compute extra parameters for the QoE score(jitter,throughput,etc). After this , the final QoE prediction is made using a lightweight machine learning model on the switch itself.
