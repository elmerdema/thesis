...
#let reporter_architecture(image_path) = [
  ...
  The processing flow begins in the Data Plane when raw packets enter the ingress pipeline. For every incoming packet, the REPORTER updates Feature Registries with new telemetry data. A subset of these extracted features is then used by a *#abbr("rf") (#abbr("rf")) Classifier*, which is encoded directly into the reporter's hardware. This classifier determines if a packet belongs to a video flow.
