#let telemetry_parsing_results() =[
  ...
  #figure(
    table(
      ...
      fill: (col, row) => if row == 0 { luma(230) } else { none },[*Traffic Metric*], [*Count*],
      
      [Total Captured Packets],[20,263],
      [Background / Other Packets], [20,228],[DTA Telemetry Reports generated], [*35*]
    ),
    caption:[Summary of the captured PCAP file during the Reporter testing phase.]
  ) <tab:pcap_summary>

  Of the 20,263 packets captured, the switch successfully emitted 35 dedicated DTA telemetry reports (one every 100ms as configured). To validate the hardware logic, the payload of the telemetry reports was fully decoded. Table 2 details the specific fields extracted from Report \#1. 
  ...
  #figure(
    table(
      ...
      [Packet Count], [198],
      [Last Packet Timestamp], [850,630,301],[Sum of IAT], [99,952,039],[Sum of IAT² / IAT³], [0 / 0],
      [Sum of Packet Size],[272,844],
      [Sum of Packet Size² / Size³], [0 / 0],[Jitter], [2,001,694],
