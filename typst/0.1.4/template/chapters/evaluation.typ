...
#let evaluation() = [
  ...
  The first major experiment evaluates the impact of the number of telemetry features ($k$) on both Machine Learning (ML) performance and hardware resource utilization. The sweep was conducted using a fixed configuration of 2 trees and depth 2, isolating the effect of feature count from other hyperparameters.
  ...
  Figure 17 consolidates all successfully compiled configurations into a unified resource view. The top-left memory panel shows that feature-sweep configurations (feat\_k3 through feat\_k8) consume a uniformly low SRAM and TCAM footprint of roughly 24 units total, while tree/depth sweep configurations scale substantially: 4t\_d4 peaks at approximately 112 units of combined SRAM and TCAM. TCAM dominates the memory budget in deeper configurations because each additional tree depth doubles the number of ternary match entries required per stage. The top-right panel shows that VLIW instruction counts and logical table IDs track the number of trees and depth closely: configurations such as 4t\_d3 and 5t\_d2 exceed 100 VLIW instructions, creating significant pressure on the instruction word budget. The bottom-left panel plots active pipeline stages and the critical-path length across all configurations. Feature-sweep entries cluster at 8 active stages regardless of $k$, while deeper tree configurations progressively grow toward the 20-stage maximum. The critical path, marked in red, frequently exceeds the 16-stage soft cap for configurations with depth $\geq$ 3, explaining the compiler timeouts observed in the previous analysis. Finally, the bottom-right scatter plot maps test accuracy against total SRAM, with bubble size encoding active pipeline stages and colour encoding VLIW instruction count. The Pareto-optimal region -- highest accuracy at lowest resource cost -- is occupied by configurations feat\_k6 (accuracy 0.761, SRAM ~8) and 4t\_d2 (accuracy 0.763, SRAM ~14). Configurations such as 3t\_d4 (accuracy 0.756, SRAM ~16) and 6t\_d2 (accuracy 0.756, SRAM ~21) represent poor trade-offs, consuming significantly more resources for lower or equal accuracy. This analysis confirms that shallow forests (depth 1--2) with a moderate number of trees (4--7) and a small, high-importance feature set (k=4--6) represent the practical sweet spot for deployable in-network QoE classification on the Tofino 2 ASIC.
