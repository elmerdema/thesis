...
#let abstract() = [
  ...
  This work hypothesizes that by using time-windowed rolling statistics and mapping a constrained Random Forest #abbr("rf") classifier into #abbr("mat"), it is possible to perform accurate, line-rate #abbr("qoe") prediction strictly within the physical memory and pipeline constraints of the Intel Tofino #abbr("asic").

  To achieve this, a hybrid architecture is introduced for real-time #abbr("qoe") monitoring that performs line-rate feature extraction and prediction. This design divides the predictive model into two components: a module deployed on a P4/Tofino switch performs initial video flow classification and extracts primitive features at line rate, while the final #abbr("qoe") prediction is made using a lightweight machine learning model on the switch itself. The main contributions include a mapping of #abbr("rf") classifiers to P4 match-action tables and a comprehensive benchmarking framework for evaluating in-network Machine Learning (#abbr("ml")) performance.
  #figure(
