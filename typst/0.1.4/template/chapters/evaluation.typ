...
#let evaluation() = [
  ...
  #figure(
    ...
    caption: [Hardware resource consumption (#abbr("sram"), #abbr("tcam"), #abbr("vliw") instructions, and pipeline stages) relative to the number of features.],
  )

  Figure 13 reveals the hardware cost of expanding the feature space on the Tofino 2 #abbr("asic"). The top-left memory panel shows that #abbr("sram") consumption remains constant at approximately 8 units across all values of $k$, while #abbr("tcam") usage stays flat at roughly 15 units, both well within the device's S8/T16 budget. The number of logical table IDs is also stable at 11 throughout the sweep, meaning the structural topology of the P4 program does not change with feature count. However, the top-center panel tells a different story for #abbr("vliw") instruction slots: these grow steeply and linearly from 23 at $k=3$ to 50 at $k=8$. Each additional feature introduces new ternary match keys into the decision-tree tables, expanding the instruction word required to evaluate each node. The bottom-left panel confirms that pipeline stage usage remains flat at 8 active stages for all tested values of $k$, staying well below both the soft cap of 16 and the hard maximum of 20, with compilation times growing moderately from 7s to 10s. The bottom-right scatter plot synthesises the accuracy-versus-#abbr("sram") trade-off: all configurations occupy approximately the same #abbr("sram") footprint (~8.0 units) yet show small but meaningful accuracy differences. Notably, $k=6$ achieves the highest test accuracy of 0.761 with the largest #abbr("vliw") footprint (bubble size), while $k=4$ drops to only 0.757. This analysis confirms that $k=4$ (the four features ps\_sum, ps2\_sum, ps3\_sum, and jitter selected in the training pipeline) represents the optimal operating point: it delivers near-peak accuracy while minimising #abbr("vliw") pressure and compilation overhead.

  However, this improvement in #abbr("ml") metrics comes at a steep cost in terms of hardware resources. The compiler reports indicate that #abbr("sram"), #abbr("tcam"), and specifically #abbr("vliw") instruction slots scale rapidly as $k$ increases. The physical limit of 20 pipeline stages on the Tofino 2 architecture becomes a hard bottleneck, as demonstrated in the stage utilization heatmaps.


  === Tree and Depth Configuration Sweep
  The second experiment systematically sweeps the number of trees (3--10) and the maximum depth (d1--d5) of the #abbr("rf"), using the fixed 4-feature set identified in the previous experiment. While larger and deeper forests naturally yield higher test accuracies and better recall, they strain the P4 compiler's ability to fit the logic within the limited hardware stages.
  ...
  The aggregated resource overview illustrates the complex multi-dimensional trade-off between test accuracy, total #abbr("sram") allocated, active pipeline stages, and #abbr("vliw") instructions. It proves that by carefully tuning the hyperparameters, an optimal configuration can be found that maximizes #abbr("qoe") prediction accuracy while remaining strictly within the hardware bounds of the Tofino 2 #abbr("asic").

  #figure(
    image("../assets/benchmarks/08_resource_overview.png", width: 100%),
    caption: [Combined resource overview mapping accuracy against #abbr("sram") usage and active stages.],
  )

  Figure 17 consolidates all successfully compiled configurations into a unified resource view. The top-left memory panel shows that feature-sweep configurations (feat\_k3 through feat\_k8) consume a uniformly low #abbr("sram") and #abbr("tcam") footprint of roughly 24 units total, while tree/depth sweep configurations scale substantially: 4t\_d4 peaks at approximately 112 units of combined #abbr("sram") and #abbr("tcam"). #abbr("tcam") dominates the memory budget in deeper configurations because each additional tree depth doubles the number of ternary match entries required per stage. The top-right panel shows that #abbr("vliw") instruction counts and logical table IDs track the number of trees and depth closely: configurations such as 4t\_d3 and 5t\_d2 exceed 100 #abbr("vliw") instructions, creating significant pressure on the instruction word budget. The bottom-left panel plots active pipeline stages and the critical-path length across all configurations. Feature-sweep entries cluster at 8 active stages regardless of $k$, while deeper tree configurations progressively grow toward the 20-stage maximum. The critical path, marked in red, frequently exceeds the 16-stage soft cap for configurations with depth $\geq$ 3, explaining the compiler timeouts observed in the previous analysis. Finally, the bottom-right scatter plot maps test accuracy against total #abbr("sram"), with bubble size encoding active pipeline stages and colour encoding #abbr("vliw") instruction count. The Pareto-optimal region -- highest accuracy at lowest resource cost -- is occupied by configurations feat\_k6 (accuracy 0.761, #abbr("sram") ~8) and 4t\_d2 (accuracy 0.763, #abbr("sram") ~14). Configurations such as 3t\_d4 (accuracy 0.756, #abbr("sram") ~16) and 6t\_d2 (accuracy 0.756, #abbr("sram") ~21) represent poor trade-offs, consuming significantly more resources for lower or equal accuracy. This analysis confirms that shallow forests (depth 1--2) with a moderate number of trees (4--7) and a small, high-importance feature set (k=4--6) represent the practical sweet spot for deployable in-network #abbr("qoe") classification on the Tofino 2 #abbr("asic").
  ...
  The central hypothesis of this thesis, that a constrained #abbr("rf") classifier
  mapped to #abbr("mat") can perform accurate, line-rate #abbr("qoe") prediction
  ...
  Tofino Native Architecture (TNA). The automated pipeline translates a trained scikit-learn #abbr("rf") directly into compilable P4 code,
  ...
  bottleneck, not #abbr("sram") or #abbr("tcam"). Configurations with depth $d > 3$ and more than
  ...
  *At\_Risk* vs. 38.7% *Steady*) reflects the nature of the dataset: video streaming
  ...
  Furthermore, the deployment of a single unified #abbr("rf") introduces a trade-off between model simplicity and classification precision during
